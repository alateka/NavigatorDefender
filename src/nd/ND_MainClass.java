...
//=============================( Main Class )=================================================================//
	...
	public static LANG language = new LANG("A properties file will be generated.",
			...
			"The file ", " it has not viruses", "Detected Viruses on: ");
	...
	public static void main(String[] args) {
		...
		try { // If the ND.properties file exist, then it is going to load its settings.
			...
			JOptionPane.showMessageDialog(null, language.getPropertiesFileGenerated(), "NavigatorDefender",
					JOptionPane.INFORMATION_MESSAGE);

			if (osVersion.contains("Linux")) { // If it's Linux, the ND.properties file will add Downloads folder for
											   // GNU/Linux users.
				
				propertiesFile.setProperty("Downloads_directory",
						System.getProperty("user.home") + "/" + TOOLS.downloadsDir());
				
				propertiesFile.setProperty("LANG", System.getProperty("user.language"));

			} else if (osVersion.contains("Windows")) { // If it's Windows, then the ND.properties file will add
														// Downloads folder for Windows users.
		...
		if (propertiesFile.getProperty("LANG").equals("es")) {
			...
			language.setNavigatorDefenderBackground("NavigatorDefender esta en segundo plano."
		...
		JOptionPane.showMessageDialog(null, language.getNavigatorDefenderBackground(), "NavigatorDefender",
				JOptionPane.INFORMATION_MESSAGE, iconND);
		...
		try {
			while (FileDownloaded.take() != null) {
				...
				for (WatchEvent<?> event : key.pollEvents()) {
					...
					// Browsers will generate temporary files during a download,
					// so it have not to be scan.
					if (event.context().toString().contains(".crdownload")
						...
						if (eventsCount == 2) {
							WindowsMode.runWindowsDefender(dowloadDir.toString(), 
